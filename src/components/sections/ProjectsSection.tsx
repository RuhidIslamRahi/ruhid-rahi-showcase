      ...
      <div className="container px-4">
        <div className="text-center mb-16">
          <div className="relative inline-block">
            <h2
              className="text-5xl md:text-6xl font-extrabold tracking-tight bg-gradient-to-r from-purple-400 via-pink-500 to-indigo-400 bg-clip-text text-transparent animate-fade-in"
        ...
        <div className="flex justify-between items-center mb-10">
          ...
          <div className="flex gap-2">
            ...
            <button 
              ...
              className={`p-2 rounded-md ${viewMode === 'carousel' ? 'bg-purple-600' : 'bg-gray-800'}`}
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-4">
                    <div className="flex gap-2">
                      {project.tags.slice(0, 3).map(tag => (
                        <span key={tag} className="bg-purple-600/70 text-white px-2 py-1 rounded-full text-xs">
                          {tag}
                        </span>
                      ))}
                      {project.tags.length > 3 && (
                        <span className="bg-gray-700/70 text-white px-2 py-1 rounded-full text-xs">
                          +{project.tags.length - 3}
                        </span>
                      )}
                    </div>
                ...
                <CardFooter className="flex gap-2 pt-0">
                  {project.demoUrl && (
                    <a 
                      ...
                      className="flex items-center gap-1 text-xs bg-purple-600 hover:bg-purple-700 text-white py-1 px-3 rounded-full transition-colors"
                    >
                  ...
                  {project.codeUrl && (
                    <a 
                      ...
                      target="_blank" 
                      rel="noopener noreferrer" 
                      className="flex items-center gap-1 text-xs bg-gray-800 hover:bg-gray-700 text-gray-300 py-1 px-3 rounded-full transition-colors"
                    >
                      <Github size={12} /> Code
                    </a>
                  )}
                        ...
                        <div className="relative">
                          ...
                          <div className="absolute top-2 right-2 flex flex-wrap justify-end gap-1 max-w-[80%]">
                            {project.tags.slice(0, 2).map(tag => (
                              <span key={tag} className="bg-purple-600/80 text-white px-2 py-0.5 rounded-full text-xs">
                                {tag}
                              </span>
                            ))}
                          </div>
