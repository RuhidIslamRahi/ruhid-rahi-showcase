    ...
    <section id="projects" className="py-20 bg-background">
      <div className="container px-4">
        <div className="text-center mb-16">
          ...
          {/* ... keep underline gesture keyframes/style ... */}
          <style>
            {`
        ...
        <div className="flex justify-between items-center mb-10">
          <div className="flex flex-wrap gap-2">
            {allTags.map(tag => <button key={tag} onClick={() => setFilter(tag)} className={`px-4 py-2 rounded-full transition-all duration-300 text-sm ${filter === tag ? 'bg-purple-600 text-white' : 'bg-muted text-foreground hover:bg-accent'}`}>
                {tag}
              </button>)}
          </div>
          
          <div className="flex gap-2">
            <button onClick={() => setViewMode('grid')} className={`p-2 rounded-md ${viewMode === 'grid' ? 'bg-purple-600' : 'bg-muted'}`} aria-label="Grid view">
              <Database size={16} />
            </button>
            <button onClick={() => setViewMode('carousel')} className={`p-2 rounded-md ${viewMode === 'carousel' ? 'bg-purple-600' : 'bg-muted'}`} aria-label="Carousel view">
              <ChartBar size={16} />
            </button>
          </div>
        </div>
        ...
        {viewMode === 'grid' ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mt-8">
            {filteredProjects.map(project => (
              <Card key={project.id} className="bg-card border-border hover:border-purple-500 transition-all duration-300 overflow-hidden group">
                ...
                <CardContent className="pt-4">
                  ...
                  <p className="text-foreground dark:text-gray-400 text-sm line-clamp-3">{project.description}</p>
                </CardContent>
                ...
                <CardFooter className="flex gap-2 pt-0">
                  ...
                  {project.codeUrl && <a href={project.codeUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-xs bg-muted hover:bg-accent text-foreground py-1 px-3 rounded-full transition-colors">
                      <Github size={12} /> Code
                    </a>}
                </CardFooter>
              </Card>
          ...
          <div className="mt-8">
            <Carousel className="w-full">
              ...
              <div className="hidden md:block">
                <CarouselPrevious className="-left-12 bg-muted hover:bg-purple-600 border-0" />
                <CarouselNext className="-right-12 bg-muted hover:bg-purple-600 border-0" />
              </div>
            </Carousel>
          </div>
        )}
